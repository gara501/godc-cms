  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: 'Cra 8a #17-22 Centro de Bogotá. - Teléfono: 2222222',
      },
    ],
  },
]

type SeedPage = (typeof pages)[number]
type ExistingPage = {
  id: number | string
}
type SeedPayload = {
  updateGlobal: (args: { slug: string; data: typeof siteSettings }) => Promise<unknown>
  find: (args: {
    collection: 'pages'
    where: { slug: { equals: string } }
    limit: number
  }) => Promise<{ docs: ExistingPage[] }>
  update: (args: { collection: 'pages'; id: ExistingPage['id']; data: SeedPage }) => Promise<unknown>
  create: (args: { collection: 'pages'; data: SeedPage }) => Promise<unknown>
}
...
console.log('Frontend content seeded in Payload CMS.')
